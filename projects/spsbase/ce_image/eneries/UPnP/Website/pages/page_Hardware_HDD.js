        ...
        this.HDD = function () {
            ...
            var OnInitStaticPage = function (RequestParamIDs) {
                ...
                if (RequestParamIDs[IDX_DSKMGNT_DRIVELETTER].getHasValues() ||
                    RequestParamIDs[IDX_DSKMGNT_VOLUMELABEL].getHasValues() ||
                    wince) // CE may have no drives (since the OS runs in memory)
                {
                    if (winxp) {
            ...
            var OnDisplayValues = function (RequestParamIDs) {
                    ...
                    wince) // CE may have no drives (since the OS runs in memory)
                {
                    ...
                    var Rows = Math.max(RequestParamIDs[IDX_DSKMGNT_DRIVELETTER].values[0].length,
                                        RequestParamIDs[IDX_DSKMGNT_VOLUMELABEL].values[0].length);
