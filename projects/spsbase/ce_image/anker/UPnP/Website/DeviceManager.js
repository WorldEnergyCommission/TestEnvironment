(function (window) {
    ...
    var DevMan = new (function () {
            ...
            if (LoadingModule > -1) {
                ...
                if (ModuleState.hasError) {
                    if (window.Helper.getStartSecurityWizard()) {
                        Helper.ShowLoading("Initializing...");
                    }
                    else {
                        Helper.ShowLoading();
                        ErrorQueue.AddError(ModuleState.error.requestStatus, ModuleState.error.requestStatusText, 30);
                    }
                }

                setTimeout(window.DevMan.Start, 1000);
            }
            else {
        ...
        var ShowLogoutButton = function () {
            ...
            {
                try {
                    document.getElementById("TitleLogout").innerHTML = new ControlLib.SmallButton().Create("btnConsole", "configure", "Web console") +
                                                                       new ControlLib.SmallButton().Create("btnLogout", "power", "Logout");

                    document.getElementById("btnConsole").onclick = function () { NavigateConsole(); };
        ...
        var NavigateConsole = function () {
            window.location.href = window.location.protocol + '//' + window.location.host + "//console";
        }
        ...
        var DoLogin = function () {
            window.location.href = window.location.protocol + '//' + window.location.host + "//config";
        }
                    ...
                    if (_req.status == 200) {
                        ...
                        try {
                            ...
                            if (_req.responseXML != null &&
                                ...
                                if (tcbsd && Helper.getIsAutheliaRedirect(_req.responseURL)) {
                                    // authelia session is expired
                                    DoLogin();
                                }
                                else {
                                    soap_resp = new dataStream.Response(true,
                            ...
                            soap_resp = new dataStream.Response(true,
                                                                new dataStream.RequestError(e.number,e.message),
                                                                undefined,
                                                                false);
                        }
