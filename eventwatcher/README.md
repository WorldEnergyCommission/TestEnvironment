...
For multi platform usage (e.g. DEV and TSG) we target multiple credentials/queues because: 
- accessToken invalidiation: when refreshing the token on one platform (e.g. DEV) the previous accessToken retrieved on another platform (e.g. Effectas) is invalidated. This is currently not the case with easee but might be subject to change
