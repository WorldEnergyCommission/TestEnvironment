...
class StreamAnomalyDetectionDataHolder(DataHolder):
    def __init__(self, device_id: str, collection_id: str, project_id: str, heart_beat: str, error: str, ready: str,
                 name: str, created_at: str, favorite: bool, error_rule: str, warning_rule: str, anomaly_score: str,
                 chart_options: list[ChartOption], site_id: int, username: str, password: str, width: str,
                 threshold: float, type_name: str = "StreamAnomalyDetection") -> None:
        """creates the stream anomaly detection data holder class, which contains static information which is needed to
    ...
    @classmethod
    def from_dict(cls, d: dict[str, ...]) -> StreamAnomalyDetectionDataHolder:
        ...
        return cls(project_id=d["project_id"], device_id=d["id"], collection_id=d["collection_id"],
                   ...
                   anomaly_score=controller_mappings["anomalyScore"],
                   chart_options=chart_options, threshold=d["data"]["threshold"],
                   width=d["data"]["selectedWidth"]
                   )
    ...
    def get_data(self, start: int, end: int, add_minutes_of_day: bool=True, **kwargs) -> pd.DataFrame:
        """get data for the model"""

        df = pd.DataFrame()
        for v in self.variables:
            data = fetch_data_from_http(v, self.project_id, start, end, "1m") # TODO hardcoded sampling
            df = append_data_to_dataframe_http(df, data, v)
        df = df.set_index("ts")
        if add_minutes_of_day:
