<template>
  <div class="consumption-service-mqtt-charts">
...
export default defineComponent({
  ...
  props: {
    ...
    isPreview: { default: true, type: Boolean },
    mqttChartsVariables: {
      default: () => ({
        actualPower: "",
  ...
  computed: {
    langPath() {
      return "mlModel.ConsumptionService.settingsView";
    },
    /**
        ...
        data: {
          ...
          options: {
            value_1: {
              agg: "avg",
              name: this.$t(
                "mlModel.ConsumptionService.settingsView.chartLabels.power.actualPower",
              ),
              scaling: {
            ...
            value_2: {
              agg: "avg",
              name: this.$t(
                "mlModel.ConsumptionService.settingsView.chartLabels.power.calculatedPower",
              ),
              scaling: {
                max: this.chartsScaling * 2,
                min: 0 - this.chartsScaling * 2,
...
<style lang="scss">
.consumption-service-charts {
  .consumption-service-mqtt-charts {
    min-height: 560px;
