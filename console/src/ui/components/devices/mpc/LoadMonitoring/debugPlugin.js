...
function inRangeByAxis(element, event, axis) {
  if (axis !== "x" && axis !== "y") {
    return (
      element.inRange(event.x, event.y, "x", true) || element.inRange(event.x, event.y, "y", true)
    );
  }
...
function inPointRange(point, center, radius, borderWidth) {
  ...
  return (
    ...
    Math.pow(radius + hBorderWidth, 2)
  );
}
...
const fontsKey = (fonts) =>
  fonts.reduce(function (prev, item) {
    prev += item.string;
    return prev;
  }, "");
...
function measureLabelSize(ctx, options) {
  ...
  const mapKey =
    lines.join() + fontsKey(fonts) + strokeWidth + (ctx._measureText ? "-spriting" : "");
...
function calculateX({ properties, options }, labelSize, position, padding) {
  ...
  return calculatePosition$1(
    { start, end, size, borderWidth: options.borderWidth },
    {
      position: position.x,
...
function dispatchMoveEvents({ state, event }, hook, elements, checkElements) {
  ...
  for (const element of elements) {
    if (checkElements.indexOf(element) < 0) {
      changed =
        dispatchEvent(element.options[hook] || state.listeners[hook], element, event) || changed;
    }
  }
  return changed;
}
...
const angleInCurve = (start, cp, end, t) =>
  -Math.atan2(
    coordAngleInCurve(start.x, cp.x, end.x, t),
    coordAngleInCurve(start.y, cp.y, end.y, t),
  ) +
  0.5 * PI;
  ...
  inRange(mouseX, mouseY, axis, useFinalPosition) {
    ...
    if (axis !== "x" && axis !== "y") {
      ...
      if (path) {
        ...
        const result =
          ctx.isPointInStroke(path, mx, my) || isOnLabel(this, point, useFinalPosition);
        ctx.restore();
        return result;
      }
...
function inAxisRange(element, { mouseX, mouseY }, axis, { hBorderWidth, useFinalPosition }) {
  ...
  return (
    (limit.value >= limit.start - hBorderWidth && limit.value <= limit.end + hBorderWidth) ||
    isOnLabel(element, { mouseX, mouseY }, useFinalPosition, axis)
  );
}
...
function calculateLabelPosition(properties, label, sizes, chartArea) {
  ...
  const p2 = { x: properties.x2, y: properties.y2 };
  const rotation =
    label.rotation === "auto" ? calculateAutoRotation(properties) : toRadians(label.rotation);
...
function calculateT(properties, label, sizes, chartArea) {
  ...
  if (label.position === "start") {
    t = calculateTAdjust(
      { w: properties.x2 - properties.x, h: properties.y2 - properties.y },
      sizes,
      label,
      space,
    );
    ...
    t =
      1 -
      calculateTAdjust(
        { w: properties.x - properties.x2, h: properties.y - properties.y2 },
        sizes,
        label,
        space,
      );
  ...
  inRange(mouseX, mouseY, axis, useFinalPosition) {
    ...
    if (axis !== "x" && axis !== "y") {
      return pointInEllipse(
        { x: mouseX, y: mouseY },
        this.getProps(["width", "height", "centerX", "centerY"], useFinalPosition),
        rotation,
        borderWidth,
      );
    }
    ...
    return (
      rotatedPoint[axis] >= limit.start - hBorderWidth - EPSILON &&
      rotatedPoint[axis] <= limit.end + hBorderWidth + EPSILON
    );
  }
...
function pointInEllipse(p, ellipse, rotation, borderWidth) {
  ...
  return (
    a / Math.pow(xRadius + hBorderWidth, 2) + b / Math.pow(yRadius + hBorderWidth, 2) <= 1.0001
  );
}
...
class PointAnnotation extends Element {
  inRange(mouseX, mouseY, axis, useFinalPosition) {
    ...
    const hBorderWidth = borderWidth / 2;
    const limit =
      axis === "y" ? { start: y, end: y2, value: mouseY } : { start: x, end: x2, value: mouseX };
  ...
  inRange(mouseX, mouseY, axis, useFinalPosition) {
    if (axis !== "x" && axis !== "y") {
      return (
        ...
        pointIsInPolygon(this.elements, mouseX, mouseY, useFinalPosition)
      );
    }
    const rotatedPoint = rotated(
      { x: mouseX, y: mouseY },
...
function updateElements(chart, state, options, mode) {
  ...
  for (let i = 0; i < annotations.length; i++) {
    ...
    if (!annotationOptions.setContext) {
      continue;
    }
...
const annotation = {
  ...
  afterUpdate(chart, args, options) {
    ...
    state.visibleElements = state.elements.filter((el) =>
      el && el.options ? !el.skip && el.options.display : false,
    );
...
function draw(chart, caller, clip) {
  ...
  //TODO: here is the fix for the plugin to work..
  if (clip & chartArea) {
    clipArea(ctx, chartArea);
  }

  if (state) {
    const drawableElements = getDrawableElements(state.visibleElements, caller).sort(
      (a, b) => a.element.options.z - b.element.options.z,
    );
