<template>
  <div ref="canvas" class="setpoint-optimizer-actual-view">
    <div class="setpoint-optimizer-actual-view__title">
      {{ $t("mlModel.SetpointOptimizer.mainView.actualView") }}
    </div>
...
import SystemInstance from "@/ui/components/devices/mpc/SetpointOptimizer/system/components/SystemInstance.vue";
import NavigationTab from "@/ui/components/modals/components/NavigationTab.vue";

/**
 * Component that represent actual view of setpoint optimizer system
 */
export default defineComponent({
  ...
  watch: {
    ...
    containerWidth: [{ handler: "getInstanceCoordsOnNextTick" }],
    containerHeight: [{ handler: "getInstanceCoordsOnNextTick" }],
  },
  async created() {
    this.tab = 0;
  ...
  methods: {
    ...
    getInstanceCoordsOnNextTick() {
      this.$nextTick(() => this.getInstancesCoords());
    },
    /**
...
.setpoint-optimizer-actual-view {
  &__title {
    text-align: center;
    font-size: 20px;
  }
