...
/**
 * Component that represent Play Basic Control
 */
export default defineComponent({
  extends: BaseDevice,
  props: {
    color: { default: "#7c7c7c", type: String },
    icon: { default: "play", type: String },
  },
  data() {
    return {
      /**
  ...
  created() {
    if (this.measurementsClean.commandPlay === "") {
      this.isNotMapped = true;
    }
  },
  methods: {
    down() {
      this.send([{ v: 1, n: this.measurementsClean.commandPlay, u: "" }]);
    },
    up() {
      this.send([{ v: 0, n: this.measurementsClean.commandPlay, u: "" }]);
    },
