<template>
  <CoreButton
    :class="['arrow-down-base', { active: state }]"
...
/**
 * Component that represent ArrowDown Basic Control
 */
export default defineComponent({
  ...
  created() {
    if (this.measurementsClean.state === "" || this.measurementsClean.down === "") {
      this.isNotMapped = true;
    }
  },
  methods: {
    down() {
      this.send([{ v: 1, n: this.measurementsClean.down, u: "" }]);
      this.send([{ v: 0, n: this.measurementsClean.down, u: "" }]);
    },
    handleIsMapped() {
      if (this.measurementsClean.state !== "" || this.measurementsClean.down !== "") {
