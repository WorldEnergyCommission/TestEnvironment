...
/**
 * Component that represent Reverse Basic Control
 */
export default defineComponent({
  extends: BaseDevice,
  props: {
    color: { default: "#7c7c7c", type: String },
    icon: { default: "previous", type: String },
    /**
  ...
  created() {
    if (this.measurementsClean.commandReverse === "") {
      this.isNotMapped = true;
    }
  },
  methods: {
    down() {
      this.send([{ v: 1, n: this.measurementsClean.commandReverse, u: "" }]);
    },
    up() {
      this.send([{ v: 0, n: this.measurementsClean.commandReverse, u: "" }]);
    },
    handleIsMapped() {
      if (this.measurementsClean.commandReverse !== "") {
