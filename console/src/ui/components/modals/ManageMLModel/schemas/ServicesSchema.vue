...
<template>
  <div class="services-schema h-100">
            ...
            <CoreContainer fluid>
              <CoreRow>
                <CoreColumn offset-sm="3" sm="6">
                  ...
                  <div class="mt-4">
                    <InputFieldNumber
                      v-model="chartsScaling"
                      :field-rules="[rules.fieldMoreThanNull, rules.twoComas]"
                      :is-decimal="true"
                      :label="$t(`mlModel.${mlModel.data.type}.scaling`)"
                      :max="10000"
                      :min="0"
                      ::value="0.01"
                    />
...
export default defineComponent({
  ...
  computed: {
    ...
    settingsValidation() {
      return !!this.mlModel.collection_id.length && !!this.chartsScaling?.length;
    },
    mlModelSchema() {
  ...
  async created() {
    ...
    if (this.isEditModal) {
      ...
      // scaling
      this.chartsScaling = this.deviceData.data?.meta?.chartScaling;
    }
  },
  methods: {
    /**
    ...
    async sendForm() {
      ...
      copy.data.meta.chartScaling = this.chartsScaling;
