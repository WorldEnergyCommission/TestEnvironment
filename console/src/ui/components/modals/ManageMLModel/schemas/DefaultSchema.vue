...
<template>
  <div class="default-mpc-schema h-100">
                      ...
                      >
                        ...
                        >
                          ...
                          <component
                            ...
                            :starting-date="defineStartDate(inputField)"
                            @date-picker-custom-clear-date="datePickerCustomClearDate(inputField)"
                          />
                          <div
                            v-if="['endDate', 'startDate'].some((e) => e === inputField)"
                            style="height: 14px"
                          />
...
export default defineComponent({
  ...
  async created() {
    ...
    // init mappings if create new ml model
  ...
  methods: {
     ...
     * @param formField date picker field key
     */
    datePickerCustomClearDate(formField: any) {
      this.controllerMappingsLocal[formField] =
        this.deviceData.data.meta.controllerMappings[formField] || "";
    },
    defineStartDate(formField: any) {
      if (formField === "endDate") {
        return this.controllerMappingsLocal.startDate;
      } else {
        return null;
      }
    },
    ...
    async addRulesWhenCreateDevice(deviceData: any) {
      ...
      // create error, warning rules
      const res = await this.addRules({
        project_id: this.project.id,
        rulesList,
      });
      // save in current Ml Model device data object ids of created error, warning rules
    ...
    async addRulesWhenEditDevice(deviceData: any) {
      ...
      if (oldErrorWarningVar && !newErrorWarningVar) {
        // remove old warning, error rules if errorWarning variable was cleared
        await Promise.all([
          this.deleteRule({
            project_id: this.project.id,
    ...
    onStageChange(val: number) {
      if (val === 2) {
        // fetch measurements for mappings when go to stage 2
