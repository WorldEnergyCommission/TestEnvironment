<template>
  <div ref="refItem" class="vue-grid-item" :class="classObj" :style="styleObj">
    <slot :style="styleObj" :is-dragging="isDragging" />
...
import { getDocumentDir, EventsData } from "@/gridLayout/helpers/DOM";
import { getControlPosition, createCoreData } from "@/gridLayout/helpers/draggableUtils";
import { getColsFromBreakpoint, Breakpoints } from "@/gridLayout/helpers/responsiveUtils";
import {
  setTopLeft,
  setTopRight,
  setTransformRtl,
  setTransform,
  Layout,
} from "@/gridLayout/helpers/utils";
import useCurrentInstance from "@/gridLayout/hooks/useInstance";
...
// item dom
const refItem = ref<HTMLElement>({} as HTMLElement);
...
let interactObj = null as Interactable | null;
// computed
...
function tryMakeDraggable() {
  if (interactObj === null || interactObj === undefined) {
    interactObj = interact(refItem.value);
  ...
  if (draggable.value && !props.static) {
    ...
    // TODO: change?
    interactObj.draggable(opts as any);
...
function tryMakeResizable() {
  ...
  if (resizable.value && !props.static) {
    ...
    interactObj.resizable(opts as any);
...
function autoSize() {
  ...
  const newSize = ($slots!.default! as any)[0].elm.getBoundingClientRect();
